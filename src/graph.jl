...
function graph_from_json(
    ...
    for (index, edges) in enumerate(raw_graph["adjacency"])
        if typeof(edges[1]) <: Real
            index = edges[1]
            edges = edges[2]
            
            for edge in edges
                if edge["id"] > index
                    add_edge!(simple_graph, index, edge["id"])
                end
            end
        else 
            for edge in edges
...
function BaseGraph(
    ...
    if edge_weights != "connections"
        println("edge_weights is set to ", edge_weights, " but this ",
                "functionality has not been turned on in ",
                "get_log_linking_edge_prob_adjacent; exiting...")
        @assert false
    end
    ...
    if uppercase(extension) == ".JSON"
        return graph_from_json(filepath, pop_col, inc_node_data, edge_weights,
                               area_col, vap_col, bpop_col, bvap_col, 
